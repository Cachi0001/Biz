...
def get_dashboard_stats():
    try:
        ...
        invoices_result = supabase.table('invoices').select('*', count='exact').eq('user_id', user_id).execute()
        ...
        # Pending invoices
        pending_invoices = supabase.table('invoices').select('*', count='exact').eq('user_id', user_id).eq('status', 'pending').execute()
