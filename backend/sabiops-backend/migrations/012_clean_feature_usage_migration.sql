-- Clean Feature Usage Migration - Version 012
-- Run this directly in Supabase SQL Editor
-- This fixes all the constraint and function issues

-- Step 1: Drop all existing objects with CASCADE to handle dependencies
DROP TRIGGER IF EXISTS update_feature_usage_timestamp_trigger ON feature_usage;
DROP FUNCTION IF EXISTS get_current_usage_stats(UUID) CASCADE;
DROP FUNCTION IF EXISTS increment_feature_usage(UUID, TEXT, INTEGER) CASCADE;
DROP FUNCTION IF EXISTS update_feature_usage_timestamp() CASCADE;

-- Step 2: Drop existing tables/views and recreate
DROP VIEW IF EXISTS usage_alerts_view CASCADE;
...
-- Step 8: Create usage stats function
...
-- Step 13: Verification and test
SELECT 
    'Migration 012 completed successfully!' as status,
    COUNT(*) as users_initialized,
    COUNT(DISTINCT feature_type) as feature_types_created
FROM feature_usage;

-- Final success message
SELECT 'Schema is now ready for subscription day tracking!' as final_status;