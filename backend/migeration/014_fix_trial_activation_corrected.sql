-- Fix trial activation for existing users and ensure proper setup (CORRECTED)
...
BEGIN
    ...
    FOR user_record IN 
        ...
        FROM public.users 
        WHERE role = 'Owner' 
        AND id NOT IN (SELECT DISTINCT user_id FROM public.feature_usage WHERE user_id IS NOT NULL)
    LOOP
        ...
        (user_record.id, 'invoices', 0, (plan_limits->>'invoices')::INTEGER, period_start, period_end, current_time, current_time),
        (user_record.id, 'expenses', 0, (plan_limits->>'expenses')::INTEGER, period_start, period_end, current_time, current_time),
        (user_record.id, 'sales', 0, (plan_limits->>'sales')::INTEGER, period_start, period_end, current_time, current_time),
        (user_record.id, 'products', 0, (plan_limits->>'products')::INTEGER, period_start, period_end, current_time, current_time);
